...
const root = await bootstrapAdmin("Backup & Restore", "backup", "Protected catalogue snapshots and recovery tools");
let domain = "catalog";
let items = [];
let busy = false;

const countLabels = { categories: "Categories", subcategories: "Subcategories", suppliers: "Suppliers", products: "Products" };
...
function render(data) {
    ...
    const rows = items.map((item, index) => `<tr>
        ...
        <td>${item.type === "snapshot" ? "Catalogue snapshot" : `${escapeHtml(item.dataset)} backup`}</td>
        <td>Catalogue</td><td class="backup-counts">${escapeHtml(countText(item.counts))}</td>
...
async function onCreate() {
    if (busy) return;
    showPageMessage("Creating and validating catalogue snapshot…", "info");
    setBusy(true);
    try { await createSnapshot(domain); showPageMessage("Catalogue snapshot created and validated."); await load(); }
...
async function onDryRun(item) {
    showPageMessage("Running full catalogue validation without making changes…", "info");
    setBusy(true);
    try {
...
async function onRestore(item) {
    ...
    form.addEventListener("submit", async (event) => {
        ...
        catch (error) { dialog.querySelector(".dialog-message").innerHTML = `<div class="alert alert-danger">${escapeHtml(error.message)}</div>`; submit.disabled = false; submit.textContent = "Restore catalogue"; }
    });
...
async function onDelete(item) {
    ...
    form.addEventListener("submit", async (event) => {
        ...
        try {
            ...
            showPageMessage("Backup item deleted. Active catalogue data and runtime media were unchanged.");
            await load();
        } catch (error) {
